...
import org.mozilla.tv.firefox.webdisplay.cursor.CursorEvent
import org.mozilla.tv.firefox.webdisplay.cursor.CursorModel

class WebDisplayHintViewModelTest {
    ...
    private lateinit var hintVM: WebDisplayHintViewModel
    private lateinit var sessionRepoState: Subject<SessionRepo.State>
    private lateinit var webDisplayDirectionEvents: Subject<CursorEvent>
    ...
    fun setup() {
        ...
        webDisplayDirectionEvents = PublishSubject.create()
        every { cursorModel.cursorMovedEvents } answers { webDisplayDirectionEvents }
        currentActiveScreen = PublishSubject.create()
        every { screenController.currentActiveScreen } answers { currentActiveScreen }

        hintVM = WebDisplayHintViewModel(sessionRepo, cursorModel, screenController, openMenuHint)
    ...
    fun pushScrolledToEdge(direction: Direction) {
        webDisplayDirectionEvents.onNext(CursorEvent.ScrolledToEdge(direction))
    }

    fun pushCursorMove(direction: Direction) {
        webDisplayDirectionEvents.onNext(CursorEvent.CursorMoved(direction))
    }
}
